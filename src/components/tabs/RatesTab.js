    ...
    return (
      <tr className="border-b" style={{ borderColor: "var(--line)" }}>
        ...
        <td className="px-3 py-2 whitespace-nowrap">
          ...
          <button onClick={() => { setDraft(rate); setEditing(false); }} aria-label="Cancel editing" className="opacity-50 hover:opacity-100 align-middle">
            <X size={14} />
          </button>
        </td>
      </tr>
    ...
    <tr className="border-b last:border-0" style={{ borderColor: "var(--line)" }}>
      ...
      {isAdmin && (
        <td className="px-3 py-2.5 text-right whitespace-nowrap">
          <button onClick={() => setEditing(true)} aria-label={`Edit ${rate.bank}`} className="opacity-40 hover:opacity-100 mr-2"><Pencil size={13} /></button>
          <button onClick={() => onDelete(rate.id)} aria-label={`Delete ${rate.bank}`} className="opacity-40 hover:opacity-100"><Trash2 size={13} /></button>
        </td>
      )}
    </tr>
  );
  ...
  return (
    <div className="ledger-card p-4 sm:p-5">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div>
          <label htmlFor="new-rate-bank" className="block text-[10px] mono opacity-60 mb-1">BANK</label>
          <input id="new-rate-bank" value={form.bank} onChange={(e) => setForm((f) => ({ ...f, bank: e.target.value }))} className="w-full text-sm border border-[var(--line)] rounded px-2 py-1.5 bg-white focus:outline-none focus:border-[var(--ledger-green-soft)]" />
        </div>
        <div>
          <label htmlFor="new-rate-apy" className="block text-[10px] mono opacity-60 mb-1">APY (%)</label>
          <input id="new-rate-apy" type="number" step="0.01" min="0" value={form.apy_pct} onChange={(e) => setForm((f) => ({ ...f, apy_pct: e.target.value }))} className="w-full text-sm mono border border-[var(--line)] rounded px-2 py-1.5 bg-white focus:outline-none focus:border-[var(--ledger-green-soft)]" />
        </div>
        <div>
          <label htmlFor="new-rate-minimum" className="block text-[10px] mono opacity-60 mb-1">MINIMUM</label>
          <input id="new-rate-minimum" value={form.minimum_note} onChange={(e) => setForm((f) => ({ ...f, minimum_note: e.target.value }))} placeholder="e.g. $0, Varies" className="w-full text-sm border border-[var(--line)] rounded px-2 py-1.5 bg-white focus:outline-none focus:border-[var(--ledger-green-soft)]" />
        </div>
        <div className="col-span-2">
          <label htmlFor="new-rate-note" className="block text-[10px] mono opacity-60 mb-1">NOTE</label>
          <input id="new-rate-note" value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} placeholder="optional" className="w-full text-sm border border-[var(--line)] rounded px-2 py-1.5 bg-white focus:outline-none focus:border-[var(--ledger-green-soft)]" />
        </div>
        <div>
          <label htmlFor="new-rate-updated" className="block text-[10px] mono opacity-60 mb-1">LAST UPDATED</label>
          <input id="new-rate-updated" type="date" value={form.last_updated} onChange={(e) => setForm((f) => ({ ...f, last_updated: e.target.value }))} className="w-full text-sm mono border border-[var(--line)] rounded px-2 py-1.5 bg-white focus:outline-none focus:border-[var(--ledger-green-soft)]" />
        </div>
        <div className="col-span-2 sm:col-span-3">
          <label htmlFor="new-rate-source" className="block text-[10px] mono opacity-60 mb-1">SOURCE URL</label>
          <input id="new-rate-source" value={form.source_url} onChange={(e) => setForm((f) => ({ ...f, source_url: e.target.value }))} placeholder="https://…" className="w-full text-sm mono border border-[var(--line)] rounded px-2 py-1.5 bg-white focus:outline-none focus:border-[var(--ledger-green-soft)]" />
        </div>
        <div className="col-span-2 sm:col-span-3">
